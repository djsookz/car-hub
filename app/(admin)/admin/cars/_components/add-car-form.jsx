...
const AddCarForm = () => {
    ...
    const carFormSchema = z.object({
        ...
        fuelType: z.string().min(1, "Fuel is required"),
   ...
   const {register, setValue, getValues,formState:{errors}, handleSubmit, watch} = useForm({
    ...
    const onSubmit = async (data) => {
        if(uploadedImages.length ===0){
            setImageError("Please upload at least on image");
            return;
        }
        const carData = {
            ...data,
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
                                    ...
                                    <Select onValueChange={(value) => setValue("status", value)}
                                        ...
                                        <SelectContent>
                                            {carStatuses.map((status) => (
                                                <SelectItem key={status} value={status}>{status.charAt(0) + status.slice(1).toLocaleLowerCase()}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                            ...
                            <div className='space-y-2'>
                                ...
                                <div>
                                    <Label htmlFor="images" className={imageError ? "text-red-500" : ""}>Images
                                        {imageError && <span className='text-red-500'>*</span>}
                                        </Label> 

                                    <div {...getMultiImageRootProps()} className={`border-2 border-dashed
                                        rounded-lg p-6 text-center cursor-pointer hover:bg-gray-50 mt-2
                                        transition ${imageError? "border-red-500" : "border-gray-300"}`}>
