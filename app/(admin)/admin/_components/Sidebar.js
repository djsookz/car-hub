    ...
    <>
        <div className='hidden md:flex h-full flex-col overflow-y-auto bg-white shadow-sm border-r'>
            {routes.map((route) => (
                ...
                className={cn(
                    "flex items-center gap-x-2 text-slate-500 text-sm font-medium pl-6 translate-all hover:text-slate-600 hover:bg-slate-100/50 h-12",
                    pathName===route.href
        ...
        <div className='md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t flex justify-around items-center h-1/6'>
            {routes.map((route) => (
                <Link key={route.href} href={route.href} 
                className={cn(
